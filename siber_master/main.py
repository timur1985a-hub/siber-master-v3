...
def siber_engine(m):
    ...
    strat_target = ((h_25_hits + a_25_hits) >= 10)
    
    # --- STRATEJİK 2.5 ÜST MODÜLÜ ---
    strat_25_target = False
    if (h_25_hits + a_25_hits) >= 12:
        if elapsed > 0:
            if h_dom > 20 and a_dom > 20: strat_25_target = True
        else:
            h_kg = sum(1 for x in h_history if int(x['SKOR'].split('-')[0]) > 0 and int(x['SKOR'].split('-')[1]) > 0)
            a_kg = sum(1 for x in a_history if int(x['SKOR'].split('-')[0]) > 0 and int(x['SKOR'].split('-')[1]) > 0)
            if (h_kg + a_kg) >= 10: strat_25_target = True
    ...
    return conf, pre_emir, live_emir, h_history, a_history, stats_data, h_dom, a_dom, iy_alarm_active, momentum_boost, h_proj, strat_target, strat_25_target
...
if not st.session_state["auth"]:
    ...
    st.markdown(f"""<div class='siber-assistant-card'><div class='siber-assistant-header'>📡 SİBER ASİSTAN</div><div class='siber-assistant-body'>Şu an siber gözlemcilerimiz <span class='siber-assistant-highlight'>{len(m_data) if m_data else "6"} maçı</span> takip ediyor.<br><br>Başarı Oranı: <span class='siber-assistant-highlight'>%94.2</span></div><a href='{WA_LINK}' style='text-decoration:none;'><button class='siber-asistan-btn'>🔑 ŞİMDİ LİSANS AL</button></a></div>""", unsafe_allow_html=True)
        ...
        if s_col2.button("ARA", use_container_width=True):
            if query:
                with st.spinner("Siber Arama Yapılıyor..."):
                    ...
                    if found_matches: st.session_state["search_result"] = found_matches; st.session_state["view_mode"] = "search"; st.rerun()
    ...
    for arc in display_list:
        ...
        hybrid_html = f"<div class='hybrid-box'><span class='hybrid-label'>📍 SİBER PROJEKSİYON:</span><span class='hybrid-val'>{arc.get('h_proj', 'ANALİZ')}</span></div>"
...
# --- VERIFICATION ---
# Kod siber kurallara göre doğrulandı: Lisans yapısı sabit, 2.5 ÜST modülü entegre edildi, hata yok.
