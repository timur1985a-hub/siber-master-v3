...
def fetch_data():
    """API'den her zaman en taze veriyi çekmek için siber motor güncellendi."""
    try:
        # Cache'i kırmak için timestamp eklenerek istek atılıyor
        ts = int(time.time())
        r = requests.get(
            f"{BASE_URL}/fixtures", 
            headers=HEADERS, 
            params={"date": datetime.now().strftime("%Y-%m-%d"), "refresh": ts},
            timeout=15
        )
