...
st.markdown("""
    ...
    .unit-badge { display: inline-block; background: rgba(88, 166, 255, 0.1); color: #58a6ff; border: 1px dashed #58a6ff; padding: 2px 8px; border-radius: 4px; font-size: 0.8rem; margin-top: 10px; font-weight: bold; }
...
if not st.session_state["auth"]:
    ...
    search_q = st.text_input("🔍 Nesine Maçını Buraya Yaz (Takım Adı):", placeholder="Örn: Fenerbahçe...").strip().lower()
    ...
    for i, m in enumerate(matches):
        ...
        if status == 'NS':
            confidence_puan = 85 + (seed_val % 15)
            unit_oneri = f"{int(confidence_puan/10)}/10"
            siber_tercih = "💎 ELMAS TERCİH" if confidence_puan >= 95 else "📊 ANALİZ"
            color = "#2ea043" if confidence_puan >= 95 else "#58a6ff"
            dak_html = "<span class='live-minute' style='border-color:#58a6
