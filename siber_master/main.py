...
# CSS BLOĞU HATASIZ HALE GETİRİLDİ
style_code = """
<style>
...
if not st.session_state["auth"]:
    ...
    for i, m in enumerate(matches):
        ...
        if is_pre:
            ...
            conf = 85 + (seed_val % 15) 
            if conf >= 95:
                s_tercih, color = "💎 ELMAS TERCİH: 2.5 ÜST & EV KAZANIR", "#2ea043"
            elif conf >= 90:
                s_tercih, color = "⭐ SİBER ELMAS: İLK YARI 0.5 ÜST", "#58a6ff"
            else:
                s_tercih, color = "📊 ANALİZ: KG VAR (RİSK DÜŞÜK)", "#f1e05a"
            ...
            conf = int(60 + (xg_total * 10))
            if conf > 99: conf = 99
            dak_html = f"<span class='live-minute'>{status if status=='HT' else f'⏱️ {elap}\''}</span>"
            if conf >= 90: s_tercih, color = "💎 SİBER TERCİH: KG VAR & 2.5 ÜST", "#2ea043"
            elif conf >= 80: s_tercih, color = "⚔️ STRATEJİK: SIRADAKİ GOL (EV)", "#f1e05a"
            else: s_tercih, color = "🚫 GÜVENLİK PASI", "#f85149"

        st.markdown(f"""
        <div class='decision-card' style='border-left: 6px solid {color};'>
            <div class='ai-score' style='color:{color};'>%{conf}</div>
            <b style='color:#58a6ff;'>⚽ {m['
