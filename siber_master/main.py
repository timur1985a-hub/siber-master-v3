...
# Geliştirilmiş Session State Başlatma
...
def fetch_data_secure():
    """Önbelleği zorla kıran ve canlı veriyi hedefleyen motor"""
    try:
        # Cache-Buster: Her istekte farklı bir ID göndererek taze veri zorlanır
        cb = str(int(time.time()))
        # Önce Canlı Maçları Zorla
        r = requests.get(f"{BASE_URL}/fixtures", headers=HEADERS, params={"live": "all", "cb": cb}, timeout=10)
        res = r.json().get('response', [])
        
        # Eğer canlı yoksa günlük fikstüre dön
        if not res:
            curr = datetime.now().strftime("%Y-%m-%d")
            r = requests.get(f"{BASE_URL}/fixtures", headers=HEADERS, params={"date": curr, "cb": cb}, timeout=10)
            res = r.json().get('response', [])
...
if not st.session_state["auth"]:
    ...
    # Giriş ekranı marquee verisi (statik kalmaması için session kontrolü)
    if not st.session_state["stored_matches"]:
        st.session_state["stored_matches"] = fetch_data_secure()
    ...
    # BUTONLAR (GÜNCELLENDİ: Doğrudan Secure Fetch tetikler)
    cx, cy = st.columns(2)
    with cx: 
        if st.button("🧹 CLEAR"): 
    ...
    if matches:
        ...
        for i, m in enumerate(filtered):
            ...
            msg = f"🔥 CANLI: {m['goals']['home']}-{m['goals']['away']} | {'SIRADAKİ GOL' if rcs_val > 70 else 'GOL RİSKLİ'}" if is_live else "🚀 ANALİZ: 1.5 ÜST / MS 1X"
        ...
        st.info("⚠️ Bekleyen veya canlı maç verisi bulunamadı. Lütfen 'UPDATE' butonunu kullanın.")
