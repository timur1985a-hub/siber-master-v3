...
# --- OTURUM KODU: BEKLENEN_GOL_POTANSIYELI_FIXED_2026 ---
...
style_code = (
    ...
    ".hybrid-box{margin-top:10px; padding:10px; background:rgba(88,166,255,0.07); border-radius:8px; border-right:5px solid #58a6ff; border-left:5px solid #58a6ff; font-size:0.85rem;}"
    ".hybrid-label{color:#8b949e; font-size:0.75rem; text-transform:uppercase; font-weight:900; display:block; margin-bottom:4px;}"
    ".hybrid-val{color:#fff; font-weight:800; font-size:0.95rem;}"
...
@st.cache_data(ttl=3600)
def check_siber_kanun_h2h_extended(h_id, a_id):
    try:
        r = requests.get(f"{BASE_URL}/fixtures/headtohead", headers=HEADERS, params={"h2h": f"{h_id}-{a_id}", "last": 1}, timeout=10)
        res = r.json().get('response', [])
        if res:
            m = res[0]
            total_h2h = (m['goals']['home'] or 0) + (m['goals']['away'] or 0)
            return total_h2h, total_h2h >= 4
        return 0, False
...
def siber_engine(m):
    ...
    h2h_last_goals, kanun_vizesi = check_siber_kanun_h2h_extended(h_id, a_id)

    def get_avg_last_5(hist):
        if not hist: return 0.0
        return sum(x['TOPLAM'] for x in hist[:5]) / 5

    h5_avg = get_avg_last_5(h_history)
    a5_avg = get_avg_last_5(a_history)
    
    # BEKLENEN GOL POTANSİYELİ (BGP) HESABI
    bgp_score = (((h5_avg + a5_avg) / 2) * 0.75) + (h2h_last_goals * 0.25)
    
    bgp_level = "DÜŞÜK"
    if bgp_score >= 3.0: bgp_level = "KRİTİK GOL BEKLENTİSİ"
    elif bgp_score >= 2.4: bgp_level = "YÜKSEK GOL BEKLENTİSİ"
    elif bgp_score >= 1.8: bgp_level = "ORTA GOL BEKLENTİSİ"
    ...
    conf = 85
    pre_emir = "ANALİZ EDİLİYOR"
    if bgp_score >= 2.8 and kanun_vizesi: pre_emir = "KESİN 2.5 ÜST"
    elif bgp_score >= 2.0: pre_emir = "KESİN 1.5 ÜST"
    else: pre_emir = "TEMKİNLİ +0.5 ÜST"

    live_emir = "VERİ BEKLENİYOR"
    if elapsed > 0:
        if (h_iy_hits + a_iy_hits) >= 12 and total == 0 and elapsed < 42:
            live_emir, conf = "KESİN İLK YARI GOL (CANLI)", 98 if momentum_boost else 94
        elif bgp_score >= 2.6 and total < 3:
            live_emir, conf = "KESİN 2.5 ÜST (CANLI)", 96 if (momentum_boost or (h_dom+a_dom)>45) else 91
        elif bgp_score >= 1.8 and total < 2:
    ...
    return conf, pre_emir, live_emir, h_history, a_history, stats_data, h_dom, a_dom, momentum_boost, h_proj, bgp_score, bgp_level
...
if not st.session_state.get("auth", False):
    ...
    st.markdown("<div class='marketing-subtitle'>Sabit Hibrit Alarm Sistemi & Beklenen Gol Potansiyeli</div>", unsafe_allow_html=True)
    ...
    with st.container():
        ...
        if s_col3.button("🔥 TOPLU TARA", use_container_width=True):
            with st.spinner("Tüm Bülten Analiz Ediliyor..."):
    ...
    if current_matches:
        for m in current_matches:
            fid = str(m['fixture']['id'])
            conf, p_emir, l_emir, h_h, a_h, s_d, h_d, a_d, m_boost, h_proj, bgp_v, bgp_l = siber_engine(m)
            st.session_state["PERMANENT_ARCHIVE"][fid] = {
                ...
                "h_h": h_h, "a_h": a_h, "stats": s_d, "h_d": h_d, "a_d": a_d, "m_boost": m_boost, "h_proj": h_proj, "bgp_v": bgp_v, "bgp_l": bgp_l
            }
    ...
    for arc in display_list:
        ...
        boost_html = "<span class='momentum-boost'>⚡ MOMENTUM UP</span>" if arc.get('m_boost') else ""
        
        # SABİT ALARM CETVELİ (BEKLENEN GOL POTANSİYELİ)
        hybrid_panel = f"""
        <div class='hybrid-box'>
            <span class='hybrid-label'>🛰️ BEKLENEN GOL POTANSİYELİ</span>
            <span class='hybrid-val'>{arc['bgp_l']} <span class='bgp-badge'>BGP: {arc['bgp_v']:.1f}</span></span>
            <div style='margin-top:5px; border-top:1px solid rgba(255,255,255,0.1); padding-top:4px;'>
                <small style='color:#8b949e;'>GÜÇ PROJEKSİYONU:</small> <b style='color:#58a6ff;'>{arc['h_proj']}</b>
            </div>
        </div>
        """
        
        st.markdown(f"<div class='decision-card' style='border-left:6px solid {card_color};'><div class='ai-score' style='color:{card_color};'>%{arc['conf']}</div><div class='live-pulse' style='display:{'inline-block' if is_live_card else 'none'}'>📡 CANLI</div>{boost_html}<br><b style='color:#58a6ff;'>{arc['league']}</b> | {arc['date']}<br><span style='font-size:1.2rem; font-weight:bold;'>{arc['home']} vs {arc['away']}</span><br><div class='score-board'>{arc['score']} <span class='live-min-badge'>{arc['min']}'</span></div><div style='display:flex; gap:10px;'><div style='flex:1; background:rgba(88,166,255,0.1); padding:5px; border-radius:5px;'><small>SİBER EMİR</small><br><b>{arc['pre_emir']}</b> {win_status}</div><div style='flex:1; background:rgba(46,160,67,0.1); padding:5px; border-radius:5px;'><small>CANLI EMİR</small><br><b>{arc['live_emir']}</b></div></div>{hybrid_panel}</div>", unsafe_allow_html=True)
        ...
        with st.expander(f"🔍 ANALİZ DETAYI: {arc['home']} vs {arc['away']}"):
            ...
            st.write("### 🏟️ Geçmiş Form Verileri")
