...
if not st.session_state["auth"]:
        ...
        if query:
            with st.spinner("Siber Uzayda Aranıyor..."):
                ...
                if res: 
                    ...
                    st.toast("Sonuç Bulunamadı!", icon="⚠️")
    ...
    with c5:
        if st.button("🧹 EKRANI TEMİZLE", use_container_width=True):
            st.session_state.update({"stored_matches": [], "view_mode": "live", "search_result": None}); st.rerun()

    display_list = []
    current_matches = []
    if st.session_state["view_mode"] == "search":
        current_matches = st.session_state.get("search_result", [])
    else:
