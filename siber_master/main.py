...
# API VE KRİTİK DEĞİŞKENLER (MÜHÜRLÜ)
...
# --- 2. TASARIM SİSTEMİ (GÖRSEL ŞABLON KORUMASI) ---
style_code = """
<style>
...
/* SİBER ORAN PANELİ - SABİT */
.siber-stats-container{background:#0d1117; border:1px solid #30363d; border-radius:12px; display:flex; justify-content:space-around; padding:30px 10px; margin:20px 0 30px 0; border-bottom:4px solid #58a6ff}
...
# --- 3. AKILLI KARAR VERICİLER (ENGINE - DOKUNULMAZ) ---
def to_tsi(utc_str):
    try:
...
def fetch_siber_data(live=True):
    try:
        # Cache'i bypass etmek için timestamp ekliyoruz (Güncelleme sorunu çözümü)
        ts = int(time.time())
        p = {"live": "all", "_ts": ts} if live else {"date": datetime.now().strftime("%Y-%m-%d"), "_ts": ts}
...
if not st.session_state["auth"]:
    ...
    if st.session_state.get("role") == "admin":
        with st.expander("🔑 ROOT KONTROL PANELİ"):
            if st.button("🔥 ARŞİVİ TEMİZLE", use_container_width=True):
                st.session_state["PERMANENT_ARCHIVE"] = {}
                st.rerun()
            search = st.text_input("Siber Token Ara:")
            for t, d in st.session_state["CORE_VAULT"].items():
                if not search or search in t:
                    cx1, cx2, cx3 = st.columns([2,1,1])
                    cx1.write(f"**{t}**")
                    cx2.write(f"`{d['pass']}`")
                    if not d['issued'] and cx3.button("AKTİF", key=t):
                        d['issued'] = True
                        st.rerun()
    ...
    if c2.button("💎 ÖNCESİ", use_container_width=True):
        ...
        st.rerun()
    if c3.button("🔄 GÜNCELLE", use_container_width=True):
        mode = True if st.session_state["view_mode"] == "live" else False
        st.session_state["stored_matches"] = fetch_siber_data(mode)
        st.rerun()
    ...
    <div class='siber-stats-container'>
        <div class='siber-stat-box'><div class='siber-stat-value' style='color:#2ea043;'>113</div><div class='siber-stat-label'>SİBER KAYIT</div></div>
    ...
    if st.session_state["view_mode"] != "clear":
        for m in st.session_state["stored_matches"]:
            ...
            <div class='decision-card'>
                ...
                <div style='background:#f85149; color:white; padding:2px 8px; border-radius:4px; display:inline-block; font-size:0.7rem; font-weight:bold;'>📡 SİSTEM AKTİF</div>
                <b style='color:#58a6ff; margin-left:10px;'>{m['league']['name']}</b>
                <div style='color:#8b949e; font-size:0.85rem; margin-top:5px;'>⚽ {m['teams']['home']['name']} vs {m['teams']['away']['name']} | ⌚ {to_tsi(m['fixture']['date'])}</div>
