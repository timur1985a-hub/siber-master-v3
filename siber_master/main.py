...
if "auth" not in st.session_state:
    st.session_state.update({
        ...
        "siber_archive": {}, # Geçmiş tahminler ve sonuçlar burada mühürlenir
        "view_mode": "live"
    })
...
.status-lost{color:#f85149;font-weight:bold;border:1px solid #f85149;padding:2px 5px;border-radius:4px}
</style>"""
...
def fetch_siber_data(live=True, date_str=None):
    try:
        params = {"live": "all"} if live else {"date": date_str or datetime.now().strftime("%Y-%m-%d")}
...
def check_success(emir, gh, ga):
    # Basit bir başarı kontrol mantığı
    total = gh + ga
    if "2.5 ÜST" in emir and total > 2: return True
    if "0.5 ÜST" in emir and total > 0: return True
    if "KG VAR" in emir and gh > 0 and ga > 0: return True
    if "KAZANIR" in emir:
        # Bu kısım ev/dep ayrımına göre detaylandırılabilir, şimdilik skor varsa True
        return True if total > 1 else False
    return False

# --- 4. GİRİŞ VE PANEL ---
if not st.session_state["auth"]:
    # (Pazarlama ve Giriş Alanı - Değişmedi)
    ...
    with c1:
        if st.button("♻️ CANLI MAÇLAR", use_container_width=True):
            data = fetch_siber_data(live=True)
            st.session_state.update({"stored_matches": data, "view_mode": "live"})
            st.rerun()
    with c2:
        if st.button("💎 MAÇ ÖNCESİ", use_container_width=True):
            data = fetch_siber_data(live=False)
            st.session_state.update({"stored_matches": data, "view_mode": "pre"})
            st.rerun()
    with c3:
        if st.button("📜 SİBER ARŞİV", use_container_width=True):
            # Arşivdeki maçların güncel durumunu çekmek için bugünkü tüm maçları tara
            data = fetch_siber_data(live=False)
            st.session_state.update({"stored_matches": data, "view_mode": "archive"})
            st.rerun()
    with c4:
    ...
    search_q = st.text_input("🔍 Takım veya Lig Ara:", placeholder="Fenerbahçe, Real Madrid...").strip().lower()

    # Filtreleme ve Analiz Döngüsü
    matches = st.session_state.get("stored_matches", [])
    if search_q:
        matches = [m for m in matches if search_q in m['teams']['home']['name'].lower() or search_q in m['teams']['away']['name'].lower() or search_q in m['league']['name'].lower()]
    ...
    for m in matches:
        ...
        # Mühürleme İşlemi (Yoksa oluştur)
        if fid not in st.session_state["siber_archive"]:
            ...
            st.session_state["siber_archive"][fid] = {
                "conf": conf, "emir": emir, "type": "LIVE" if status != 'NS' else "PRE",
                "date": m['fixture']['date'], "teams": f"{h_name} vs {a_name}"
            }

        arc = st.session_state["siber_archive"][fid]
        
        # Başarı Kontrolü (Maç bittiyse veya skor yeterliyse)
        is_finished = status in ['FT', 'AET', 'PEN']
        win_status = ""
        if is_finished or (gh+ga > 2):
            success = check_success(arc['emir'], gh, ga)
            win_status = "<span class='status-win'>✅ TUTTU</span>" if success else "<span class='status-lost'>❌ KAYIP</span>"
        ...
        st.markdown(f"""
            <div class='decision-card' style='border-left: 6px solid {color};'>
                ...
                <div class='archive-badge'>🔒 SİBER MÜHÜR: {arc['type']} ANALİZ</div> {win_status}
                <br><b style='color:#58a6ff;'>⚽ {m['league']['name']}</b> | <span class='tsi-time'>⌚ {to_tsi(m['fixture']['date'])}</span> 
                {f"<span class='live-minute'>⏱️ {elap}'</span>" if status != 'NS' else "<span class='live-minute'>BAŞLAMADI</span>"}
                ...
                <div style='margin-top:10px; padding:12px; background:rgba(46,160,67,0.1); border:1px solid {color}; border-radius:8px;'>
                    <span style='color:{color}; font-size:1rem; font-weight:900;'>🎯 ÖNERİ: {arc['emir']}</span>
                </div>
                <div class='analysis-box'>Maç Tarihi: {to_tsi(m['fixture']['date'])} | Durum: {status}</div>
            </div>
