...
# --- 0. KRİTİK SEO VE GOOGLE DOĞRULAMA (BOT GEÇİT KAPISI) ---
# Bu bölüm şifre ekranı dahil her koşulda en üstte çalışır.
st.markdown("""
    <head>
    ...
    <div style="display:none;">
        <h1>Siber Analiz AI - Yapay Zeka Futbol Analiz Sistemi</h1>
        <p>Dünya liglerinden anlık veri akışı ile stratejik maç tahminleri ve siber analizler.</p>
    </div>
...
# --- 2. DEĞİŞMEZ TASARIM SİSTEMİ (CSS) ---
style_code = (
    "<style>"
...
def siber_engine(m):
    ...
    if elapsed > 0:
        ...
        else: live_emir, conf = "MAÇ SONU +0.5", 91
    else: live_emir = "KG VAR" if is_high else "0.5 ÜST"
            ...
            for i, pkg in enumerate(["1-AY", "3-AY", "6-AY", "12-AY", "SINIRSIZ"]):
                with [t1, t2, t3, t4, t5][i]:
                    ...
                    for tk in list(subset.keys())[:15]:
                        ...
                        with col1: st.markdown(f"<div class='lic-item'><b>{tk}</b><br>Pass: {v['pass']}</div>", unsafe_allow_html=True)
                        with col2:
    ...
    if mode in ["live", "pre"] and st.session_state["stored_matches"]:
        for m in st.session_state["stored_matches"]:
            ...
            if fid not in PERMANENT_ARCHIVE:
                PERMANENT_ARCHIVE[fid] = {"fid": fid, "conf": conf, "league": m['league']['name'], "home": m['teams']['home']['name'], "away": m['teams']['away']['name'], "date": to_tsi(m['fixture']['date']), "pre_emir": p_emir, "live_emir": l_emir, "score": f"{gh}-{ga}", "status": m['fixture']['status']['short'], "min": m['fixture']['status']['elapsed']}
            else:
                if PERMANENT_ARCHIVE[fid]['status'] not in ['FT', 'AET', 'PEN']:
                    PERMANENT_ARCHIVE[fid].update({"score": f"{gh}-{ga}", "status": m['fixture']['status']['short'], "min": m['fixture']['status']['elapsed'], "live_emir": l_emir, "conf": conf})
    
    if mode == "archive": display_list = list(PERMANENT_ARCHIVE.values())
    elif mode != "clear": display_list = [PERMANENT_ARCHIVE[str(m['fixture']['id'])] for m in st.session_state.get("stored_matches", []) if str(m['fixture']['id']) in PERMANENT_ARCHIVE]
    if search_q: display_list = [d for d in display_list if search_q in d['home'].lower() or search_q in d['away'].lower()]

    for arc in display_list:
        try: gh_v, ga_v = map(int, arc['score'].split('-'))
        ...
        st.markdown(f"""<div class='decision-card'><b>{arc['home']} vs {arc['away']}</b><br>Skor: {arc['score']} | Başarı: {win_live}</div>""", unsafe_allow_html=True)
